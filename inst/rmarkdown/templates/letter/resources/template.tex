...
\else % if luatex or xelatex
  \ifxetex
  % https://tex.stackexchange.com/questions/279533/using-pandoc-and-xetex-with-tufte-handout-class-gives-fontspec-error/279741
%    \usepackage{mathspec}
  \else
    \usepackage{fontspec}
  \fi
...
$else$
\usepackage[$if(margins)$$margins$$endif$]{geometry}
$endif$
...
$if(signature)$
\usepackage{graphicx,grffile}
\signature{$if(signature-before)$\vspace*{$signature-before$}$endif$\includegraphics[width=$if(signature-width)$$signature-width$$else$0.3$endif$\textwidth]{$signature$}$if(author)$\\$if(signature-after)$\vspace*{$signature-after$}$endif$$for(author)$$author$$sep$\\$endfor$$endif$}
$else$
$if(author)$
...
% https://www.smith.edu/about-smith/college-relations/visual-identity-program/using-logo
\definecolor{smith-blue}{RGB}{0,47,95}
\definecolor{smith-gold}{RGB}{240,171,0}
...
% hack to get top margin on first page only
\vspace*{$page-one-top-margin$in}
...
% hack to get text to always move to top in short letter
$if(vertical-fill)$
\vspace*{\fill}
$endif$
\end{document}
